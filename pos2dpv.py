...
def parse_arguments():
    ...
    parser.add_argument('-p', '--pos_file',          type=str, required=True, help='KiCas POS file.')
...
def find_feeder(feeders, component):
    ...
    for feeder in feeders:
        feeder_names = [feeder["Component"]]
        if feeder["Aliases"]:
            feeder_names += feeder["Aliases"].split(":")
        if component_name in feeder_names:
            return feeder
    return None
...
def main(args):
    ...
    feeders = [feeder for feeder in csv_to_dict_list(args.feeder_setup_file) if feeder["Component"]]
    # assign each component to a feeder
    fiducials = list()
    placements = list()
    unplaced = list()
